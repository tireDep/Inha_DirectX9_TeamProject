...
void CMainGame::Update()
{
	...
	if (m_pCharacter)
	{
		m_pCharacter->Update(m_pCamera->GetCameraDirection());								// Camera + Ray(Need To modify...)
		m_pCharacter->Update(g_pTimeManager->GetElapsedTime());								// Jump + Animation
		m_pDragon->DirectionSet(m_pCamera->GetCameraDirection());							// Dragon Look Direction
		m_pDragon->Update(m_pCharacter->GetPosition(), g_pTimeManager->GetElapsedTime());	// Dragon Update
		// Color Change
		// Need To Delete Character SetColor
